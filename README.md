...
**This is the real, production deployment** — `https://noircorset.vercel.app`
is the live site customers actually use. If a GitHub Pages copy of this
repo also exists somewhere, treat it as inactive/unused; it won't stay in
sync with this one automatically, and testing against it instead of
Vercel is a common source of "I fixed that already, why is it still
broken" confusion.
...
**Product share links** — sharing a product (via the share button on
product cards, Quick View, or the product page) points at a pre-generated
static page in `/share/<id>.html`, not `product.html` directly. Each one
has real per-product Open Graph tags (title, description, and the actual
product photo where one exists) baked in at build time, so link previews
on WhatsApp/Facebook/etc. show the right product instead of a generic
site-wide fallback. Regenerate these any time `data/products.json`
changes: `SITE_URL=https://noircorset.vercel.app node
scripts/generate-share-pages.js`. There's also a Vercel-only serverless
version at `api/share.js` that does the same thing at request time instead
of build time — either works; the static version additionally works on
non-Vercel static hosts.

**`sitemap.xml` + `robots.txt`** — `sitemap.xml` is generated by
`scripts/generate-sitemap.js` from the actual product/journal data (not
hand-maintained), and currently targets `https://noircorset.vercel.app`
(confirmed as the real production domain). Regenerate after adding or
removing products/posts: `SITE_URL=https://noircorset.vercel.app node
scripts/generate-sitemap.js`. Wishlist and the bare `product.html` (no
`?id=`) are deliberately excluded — one's personal/empty by default, the
other isn't a meaningful page without a product ID.
